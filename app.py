...
if item == "Item A: Água Bruta":
    ...
    # Anotações nos pontos principais
    for i, txt in enumerate(y_p):
        ax.annotate(f"{txt:.2f}m", (x_p[i], y_p[i]), textcoords="offset points", xytext=(0,10), ha='center', fontweight='bold')
...
# ---------------------------------------------------------
    ...
    # Anotações Início e Fim LP Mista
    ax.annotate(f"{lp_mista[0]:.2f}m", (dist_x[0], lp_mista[0]), textcoords="offset points", xytext=(0,10), ha='center', fontweight='bold', color='navy')
    ax.annotate(f"{lp_mista[-1]:.2f}m", (dist_x[-1], lp_mista[-1]), textcoords="offset points", xytext=(0,10), ha='center', fontweight='bold', color='navy')
    ...
    st.write(f"**Perda de Carga Total (Mista):** {dc_misto:.2f} m")
...
# ---------------------------------------------------------
    ...
    # Anotação LP Ajustada
    ax.annotate(f"{lp2_y[0]:.2f}m", (dist_x[0], lp2_y[0]), textcoords="offset points", xytext=(0,10), ha='center', fontweight='bold', color='darkred')
    ax.annotate(f"{lp2_y[-1]:.2f}m", (dist_x[-1], lp2_y[-1]), textcoords="offset points", xytext=(0,10), ha='center', fontweight='bold', color='darkred')
    ...
    st.success(f"**Nova Cota de Partida (LP 350mm):** {nivel_final + dc_350:.2f} m")
